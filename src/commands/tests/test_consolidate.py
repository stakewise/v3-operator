...
def test_split_validators():
    ...
    assert to_from == {validator_1.public_key: [validator_2.public_key, validator_3.public_key]}
    ...
    validators = [create_validator(balance=32) for x in range(67)]

    to_from = _split_validators(validators)
    assert len(to_from[validators[0].public_key]) == 63
    assert len(to_from[validators[1].public_key]) == 2
    assert to_from == {
        validators[0].public_key: [v.public_key for v in validators[2:65]],
