...
def save_network_validators(validators: list[NetworkValidator]) -> None:
    ...
    with db_client.get_db_connection() as conn:
        conn.executemany(
            f'INSERT INTO {SettingsStore().NETWORK_VALIDATORS_TABLE} '
...
def get_last_network_validator() -> NetworkValidator | None:
    ...
    network_validators_table = SettingsStore().NETWORK_VALIDATORS_TABLE
    with db_client.get_db_connection() as conn:
        res = conn.execute(
...
def get_next_validator_index(latest_public_keys: list[HexStr]) -> int:
    ...
    with db_client.get_db_connection() as conn:
        if latest_public_keys:
            cur = conn.execute(
                f'''SELECT COUNT(*) FROM {SettingsStore().NETWORK_VALIDATORS_TABLE}
                    WHERE public_key NOT IN ({",".join(["?"] * len(latest_public_keys))})''',
                latest_public_keys,
            )
        else:
            cur = conn.execute(f'SELECT COUNT(*) FROM {SettingsStore().NETWORK_VALIDATORS_TABLE}')
...
def setup() -> None:
    ...
    with db_client.get_db_connection() as conn:
        conn.execute(
            f"""
                    CREATE TABLE IF NOT EXISTS {SettingsStore().NETWORK_VALIDATORS_TABLE} (
