...
from src.common.contracts import os_token_redeemer_contract, multicall_contract
...
async def get_vault_to_unprocessed_shares() -> dict[ChecksumAddress, Wei]:
    redeemable_positions = os_token_redeemer_contract.redeemable_positions()
    ...
    data = await ipfs_fetch_client.fetch_json(redeemable_positions.ipfs_hash)
...
async def _get_processed_shares(nonce: int, data_batch: list[dict]) -> list[Wei]:
    processed_shares_batch: list[HexStr] = []
    ...
    for data_item in data_batch:
        ...
        leaf = standard_leaf_hash(
            ...
            leaf_encoding=['uint256', 'address', 'uint256', 'address'],
        )
        processed_shares_batch.append(
            os_token_redeemer_contract.encode_abi('leafToProcessedShares', leaf)
        )

    processed_shares = await multicall_contract.aggregate(processed_shares_batch)
    return [Web3.to_int(share) for share in processed_shares]
